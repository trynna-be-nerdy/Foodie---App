 ...
 */
export async function getFlaggedPosts(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
 ...
 */
export async function getFlaggedComments(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user?.userId || req.user.role !== 'ADMIN') {
      res.status(403).json({
        success: false,
 ...
 */
export async function moderatePost(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user?.userId || req.user.role !== 'ADMIN') {
      res.status(403).json({
        success: false,
 ...
 */
export async function moderateComment(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user?.userId || req.user.role !== 'ADMIN') {
      res.status(403).json({
        success: false,
