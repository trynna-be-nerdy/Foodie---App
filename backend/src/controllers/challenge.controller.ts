...
import { getLeaderboardForScope } from '../services/leaderboard.service';
...
const leaderboardQuerySchema = z.object({
  scope: z.enum(['local', 'national', 'friends']).default('national'),
...
const createChallengeSchema = z.object({
  ...
  rewardFoodiePoints: z.number().int().min(0).default(0),
  rewardGiftCardValue: z.number().min(0).optional().nullable(),
  sponsorRestaurantId: z.string().optional().nullable(),
...
export async function getChallenges(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    let userParticipations: Map<
      string,
      { progress: number; isCompleted: boolean; completedAt: Date | null }
    > = new Map();
    ...
    const formattedChallenges = filteredChallenges.map((challenge) => {
      ...
      const requirements = challenge.requirements as { count: number; specificItems?: string[] };
 ...
 */
export async function getChallenge(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user?.userId;
    const { id } = req.params;
    ...
    if (userId) {
      participation = await prisma.challengeParticipant.findUnique({
        where: {
          challengeId_userId: {
            challengeId: id,
            userId,
 ...
 */
export async function acceptChallenge(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user?.userId;
    const { id } = req.params;
 ...
 */
export async function getChallengeProgress(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user?.userId;
    const { id } = req.params;
    ...
    const requirements = participation.challenge.requirements as { count: number };
...
export async function getLeaderboard(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const { leaderboard, currentUserRank, totalCount } = await getLeaderboardForScope({
      userId,
      scope,
      limit: limitNum,
      offset: offsetNum,
 ...
 */
export async function createChallenge(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (sponsorRestaurantId) {
      const sponsor = await prisma.restaurant.findUnique({
 ...
 */
export async function getFoodiePoints(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const userId = req.user?.userId;
    ...
    let foodiePoints = await prisma.foodiePoints.findUnique({
      where: { userId },
    });

    if (!foodiePoints) {
      foodiePoints = await prisma.foodiePoints.create({
        data: {
          userId,
          balance: 0,
          totalEarned: 0,
